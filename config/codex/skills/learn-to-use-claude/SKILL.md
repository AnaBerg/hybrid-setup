...
description: Analyze existing evidence from every intended Claude, Fable, Opus, or Sonnet model invocation, including blocked or failed calls, and propose reusable guidance improvements for explicit user approval. Applies to direct calls and calls through Claude skills or review-loop; never launches a model for its analysis.
---

# Learn to Use Claude

Learn from successful and unsuccessful Claude use without changing guidance automatically. Analyze each intended model invocation exactly once, including success, error, timeout, cancellation, empty or malformed reports, cleanup, and preflight blocking the intended call. An authentication/status check alone is not a model invocation.
...
- permissions, enabled tools, sanitized prompt and artifact references;
...
Do not invoke Claude, any `claude-*` skill, `review-loop`, subagents, benchmarks, or retries to perform this meta-analysis or validate a recommendation. Read existing evidence and applicable guidance directly. This hook does not call itself, and analysis-only reads do not create a new invocation.
...
Candidate targets are `claude-implementation`, `claude-review`, `review-loop` when relevant, applicable `AGENTS.md`, and future guidance that invokes Claude. Prefer the narrowest justified target. If no durable change is justified, say so concisely and identify any material evidence limitation.
